# GO enrichment plot using sequential time-points as the reference. Terms were manually
# selected based on GO bar plots (home/bp2582/projects/eye-reg_rnaseq
# /github/SnailEyeReg_RNASeq-pipeline/sequential-reference/03_go/output). Made by 
# Alice Accorsi
# Code written by ejr
...
map(thresholds, function(x) {
  ...
  go <- read_tsv(here(paste0("sequential-reference/03_go/output/go-breakdown_lfc", x, ".txt.gz"))) %>%
    mutate(file=cluster) %>%
    ...
    mutate(file = str_replace(file, "-seq-ref_lfc", "_lfc_")) %>%
  ...
  ggsave(here(paste0("sequential-reference/03_go/figures/select_GO_id_lfc", x, "-seq-ref.pdf")),p2, height=16, width=14)
  ...
  ggsave(here(paste0("sequential-reference/03_go/figures/select_GO_noid_lfc", x, "-seq-ref.pdf")),p3, height=16, width=14)
