...
#GO breakdown
go.br<- read.table(here("intact-reference/03_go/output/go-breakdown_lfc0.txt.gz"), sep="\t", header = TRUE, quote= "", comment.char = "")
  ...
  all.comp <- map(comp.set, function(comp){
    ...
    de.sub<- read.csv(here(paste0("intact-reference/02_dea/tables/", comp.de)), header = TRUE)
  ...
  write.table(df, file= here("intact-reference/03_go/go-degenes-search.txt.gz"), quote=FALSE, row.names = FALSE, sep="\t")
