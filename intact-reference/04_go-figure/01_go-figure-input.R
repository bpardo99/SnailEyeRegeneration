...
#GO breakdown
go<- read.table(here("intact-reference/03_go/output/go-breakdown_lfc0.txt.gz"), sep="\t", header = TRUE, quote= "", comment.char = "")
##Stages
st<- c(paste0(c(1, 3, 6, 9, 12, 15, 21, 28), "dpa"), "intact")
#Comparisons 
comp.set<- paste0("de-up-go_s_", st[1:8], "-s_intact-intact-ref_lfc0.txt.gz")
...
all.comp <- map(comp.set, function(comp){
  ...
  write.table(go.sub, file= here(paste0("intact-reference/04_go-figure/input/go-breakdown_", comp)), row.names = FALSE, quote = FALSE, sep = '\t')
  ...
  comp2<- comp %>%
    ...
    str_replace("-intact-ref_lfc0.txt", "")%>%
    str_replace("s_", "") 
  
  write.table(go.fig, file=here(paste0("intact-reference/04_go-figure/input/input_file_", comp2, ".tsv.gz")), row.names = FALSE, col.names= FALSE, 
              quote = FALSE, sep = '\t')
