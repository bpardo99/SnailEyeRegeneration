# Run DE analysis with edgeR using intact time-point as the reference.
# Brenda Pardo
# 2023-05-15
...
df.lfc <- map(lfc_list, function(lfc){
  df <- map(comp, function(x){
    ...
    write_csv(et, 
              file=here(paste0("intact-reference/02_dea/tables/eTest_",tag, ".csv.gz")))
    
    #UList of upregulated genes
    de.up <- et[which(et$FDR<= 1e-5 & et$logFC>= lfc), ]
    write_csv(de.up, 
              file=here(paste0("intact-reference/02_dea/tables/de-up_",tag, "_lfc", lfc, ".csv.gz")))
    
    #List of downregulated genes
    de.down <- et[which(et$FDR<= 1e-5 & et$logFC<= -lfc), ]
    write_csv(de.down, 
              file=here(paste0("intact-reference/02_dea/tables/de-down_",tag, "_lfc", lfc, ".csv.gz")))
    
    #List of upregulated and downregulated genes
    de.ud <- et[which((et$FDR<= 1e-5 & et$FDR>= lfc) | 
                        (et$FDR<= 1e-5 & et$FDR<= -lfc)), ]    
    write_csv(de.ud, 
              file=here(paste0("intact-reference/02_dea/tables/de-ud_",tag, "_lfc", lfc, ".csv.gz")))
    ...
    write_csv(de.up.desc, 
              file=here(paste0("intact-reference/02_dea/tables/de-up_",tag, "_lfc", lfc, "-desc.csv.gz")))
    ...
    write_csv(de.down.desc, 
              file=here(paste0("intact-reference/02_dea/tables/de-down_",tag, "_lfc", lfc, "-desc.csv.gz")))
    ...
    ggsave2(filename=here(paste0("intact-reference/02_dea/figures/volcano_", tag, "_lfc", lfc, ".pdf")), plot= volcano, width= 5, height=5)
    ...
    ma <- vm.p %>%
      ...
      theme(legend.position="none") 
    ggsave2(filename=here(paste0("intact-reference/02_dea/figures/maplot_", tag, "_lfc", lfc, ".pdf")), plot= ma, width= 5, height=5)
  ...
  write.table(de.ct, 
              file=here(paste0("intact-reference/02_dea/tables/number-de-genes-lfc", lfc,"_intact-ref.txt")),
...
pdf(file=here("intact-reference/02_dea/figures/correlation-spea-top1000.pdf"), width=20, height=20)
corrplot(cor.sp, 
                   method= "color", 
...
mds_plot <- ggplot(mds_df, aes(x=x, y=y, color=factor(timepoint, level=plot_order), label=rep1)) +
  ...
  xlim(NA, 3.5)
ggsave(filename =here(paste0("intact-reference/02_dea/figures/mds-eric-allgenes.pdf")), width=6, height=5)
...
mds_plot <- ggplot(mds_df, aes(x=x, y=y, color=factor(timepoint, level=plot_order))) +
  ...
  ggtitle(paste0("MDS PLOT: all genes")) +
  xlim(NA, 2.7)
ggsave(filename = here("intact-reference/02_dea/figures/mds-eric-nlab-allgenes.pdf"), width=6, height=5)
...
df.lfc <- map(lfc_list, function(lfc){
  df <- map(comp, function(x){
    tag <- paste0(x[2],"-", x[1], "-intact-ref")
    tab<- read.csv(here(paste0("intact-reference/02_dea/tables/de-up_",tag, "_lfc", lfc, ".csv.gz")))
    return(tab)
