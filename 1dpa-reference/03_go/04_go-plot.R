# GO enrichment plot using 1dpa time-point as the reference. Terms were manually
# selected based on GO bar plots (home/bp2582/projects/eye-reg_rnaseq
# /github/SnailEyeReg_RNASeq-pipeline/1dpa-reference/03_go/output). Made by 
# Alice Accorsi
# Code written by ejr
...
map(thresholds, function(x) {
  ...
  go <- read_tsv(here(paste0("1dpa-reference/03_go/output/go-breakdown_lfc", x, ".txt.gz"))) %>%
    mutate(file=cluster) %>%
    ...
    mutate(file = str_replace(file, "-1dpa-ref_lfc", "_lfc_")) %>%
  ...
  ggsave(here(paste0("1dpa-reference/03_go/figures/select_GO_id_lfc", x, "-1dpa-ref.pdf")),p2, height=16, width=14)
  ...
  ggsave(here(paste0("1dpa-reference/03_go/figures/select_GO_noid_lfc", x, "-1dpa-ref.pdf")),p3, height=16, width=14)
