...
files <- paste0("eTest_s_", c(1, 3, 6, 9, 12, 15, 21, 28), "dpa-s_intact-intact-ref.csv.gz")
et <- map(files, function(x){
  f<- read.csv(here(paste0("intact-reference/02_dea/tables/", x)))
})
names(et) <- files
...
st<- paste0("s", c(1,3,6,9,12,15,21,28), "dpa")
logfc.t <- logfc.t %>% dplyr::select(gene_id, all_of(st))

#Save table with logFC
write_csv(logfc.t, here("intact-reference/02_dea/tables/logFC-intact-ref.csv.gz"))
