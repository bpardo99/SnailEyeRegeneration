...
lfc<- paste0("lfc", 0:2)
files <-here(paste0("1dpa-reference/02_dea/tables/number-de-genes-", lfc, "_1dpa-ref.csv.gz"))
...
order <- c("3dpa-1dpa", "6dpa-1dpa", "9dpa-1dpa", "12dpa-1dpa", "15dpa-1dpa", 
           "21dpa-1dpa", "28dpa-1dpa", "intact-1dpa")
...
p.count <- map(lfc, function(x){
  ...
  de.ct.p.up<- de.ct.p %>%
    ...
    mutate(tag = str_replace_all(tag, "-1dpa-ref", "")) %>%
  ...
  p_up<- de.ct.p.up %>%
    ...
    ggtitle(paste0("Number of DE genes accross vs-1dpa comparisons, upregulated")) +
    xlab("Comparison") + 
  ...
  pdf(file=here(paste0("1dpa-reference/02_dea/figures/number-de-genes-up-", x, "_1dpa-ref.pdf")))
  print(p_up)
  dev.off()
  ...
  p_down<- de.ct.p.down %>%
    ...
    ggtitle(paste0("Number of DE genes accross vs-1dpa comparisons, downregulated")) +
    xlab("Comparison") + 
    ylab("Number of DE genes") +
    theme_cowplot() +
    theme(axis.text.x = element_text(angle=90),
  ...
  pdf(file=here(paste0("1dpa-reference/02_dea/figures/number-de-genes-down-", x, "_1dpa-ref.pdf")))
  print(p_down)
  dev.off()
...
pdf(file=here("1dpa-reference/02_dea/figures/pax-pw1-rho.pdf"), width = 5, height= 5)
ppr
p
dev.off()
...
de.list<- read_csv(here("1dpa-reference/02_dea/tables/de-up_lfc2.csv.gz"))
de.list<- de.list$gene_id

#Read TF list
tfs <- read_tsv(file=here("01_raw-data/putative_tfs-ejr.list"))
names(tfs)<- "gene_id"
...
#Read logFC table
lfc.1dpa<-read_csv(here("1dpa-reference/02_dea/tables/logFC-1dpa-ref.csv.gz"))
exp.tab<- list(lfc.1dpa, tpm.l1p, tpm.z)
...
a<- pmap(list(exp.tab, exp.nam, limscal), function(exp.t, exp.n, lsc){
  ...
  pdf(file=here(paste0("1dpa-reference/02_dea/figures/tfs-", exp.n, ".pdf")), width = 20, height= 15)
  print(b)
  dev.off()
})
...
a<- pmap(list(exp.tab, exp.nam, limscal), function(exp.t, exp.n, lsc){
  ...
  pdf(file=here(paste0("1dpa-reference/02_dea/figures/eye-mkrs-", exp.n, ".pdf")), width = 20, height= 15)
  print(plot)
  dev.off()
...
files<- paste0("de-up_s_", comp,"-1dpa-ref_")
...
boolean.marker<-map(lfc, function(lf){
  ...
  p <- bol.mkr %>%
    ...
    mutate(comp = str_replace(comp, "de-up_s_", ""),
           comp = str_replace(comp, "-1dpa-ref_", ""),
  ...
  pdf(file=here(paste0("1dpa-reference/02_dea/figures/eye-mkrs-boolean_", lf, ".pdf")), width = 20, height= 15)
  print(p)
  dev.off()
...
ggsave(filename=here("1dpa-reference/02_dea/figures/tf-woundhealing-div.pdf"), width=10, height= 20)
