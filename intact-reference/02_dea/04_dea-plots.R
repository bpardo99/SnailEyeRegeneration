...
order <- c("1dpa-intact", "3dpa-intact", "6dpa-intact", "9dpa-intact", "12dpa-intact", "15dpa-intact", 
           "21dpa-intact", "28dpa-intact")
...
p.count <- map(lfc, function(x){
  ...
  de.ct.p.up<- de.ct.p %>%
    ...
    mutate(tag = str_replace_all(tag, "-intact-ref", "")) %>%
  ...
  pdf(file=here(paste0("intact-reference/02_dea/figures/number-de-genes-up-", x, "_intact-ref.pdf")))
  print(p_up)
  dev.off()
  ...
  pdf(file=here(paste0("intact-reference/02_dea/figures/number-de-genes-down-", x, "_intact-ref.pdf")))
  print(p_down)
  dev.off()
...
pdf(file=here("intact-reference/02_dea/figures/pax-pw1-rho.pdf"), width = 5, height= 5)
ppr
p
dev.off()
...
de.list<- read.csv(here("intact-reference/02_dea/tables/de-up_lfc2.csv.gz"))
...
#Read logFC table
lfc.intact<-read.csv(here("intact-reference/02_dea/tables/logFC-intact-ref.csv.gz"))
exp.tab<- list(lfc.intact, tpm.l1p, tpm.z)
...
a<- pmap(list(exp.tab, exp.nam, limscal), function(exp.t, exp.n, lsc){
  ...
  pdf(file=here(paste0("intact-reference/02_dea/figures/tfs-", exp.n, ".pdf")), width = 20, height= 15)
  print(b)
  dev.off()
})
...
a<- pmap(list(exp.tab, exp.nam, limscal), function(exp.t, exp.n, lsc){
  ...
  pdf(file=here(paste0("intact-reference/02_dea/figures/eye-mkrs-", exp.n, ".pdf")), width = 20, height= 15)
  print(plot)
  dev.off()
...
boolean.marker<-map(lfc, function(lf){
  ...
  p <- bol.mkr %>%
    ...
    mutate(comp = str_replace(comp, "de-up_s_", ""),
           comp = str_replace(comp, "-intact-ref_", ""),
  ...
  pdf(file=here(paste0("intact-reference/02_dea/figures/eye-mkrs-boolean_", lf, ".pdf")), width = 20, height= 15)
  print(p)
  dev.off()
...
ggsave(filename=here("intact-reference/02_dea/figures/tf-woundhealing-div.pdf"), width=10, height= 20)
