...
go.lfc <-map(lfc_list, function(lfc){
  ...
  go <- map(file_list, function(x) {
    ...
    enrich_out <- map(c("MF", "CC", "BP"), function(y) {
      ...
      if (!is.null(ego)) {
        ...
        filename = paste0("03_go/output/", x, y, "_bar.png")
  ...
  write_tsv(df, 
          file= here(paste0("sequential-reference/03_go/output/go-breakdown_lfc", lfc, ".txt.gz")))
})
