...
go<- read.table(here("1dpa-reference/03_go/output/go-breakdown_lfc0.txt.gz"), sep="\t", header = TRUE, quote= "", comment.char = "")
...
all.comp <- map(comp.set, function(comp){
  ...
  write.table(go.sub, file= here(paste0("1dpa-reference/04_go-figure/input/go-breakdown_", comp)), row.names = FALSE, quote = FALSE, sep = '\t')
  ...
  #write_tsv(go.fig, file=here(paste0("1dpa-reference/04_go-figure/input/input_file_", comp2, ".tsv.gz")), row.names = FALSE, col.names= FALSE, 
              #quote = FALSE)
  write.table(go.fig, file=here(paste0("1dpa-reference/04_go-figure/input/input_file_", comp2, ".tsv.gz")), row.names = FALSE, col.names= FALSE, 
              quote = FALSE, sep = '\t')
})
